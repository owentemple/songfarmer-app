...
1. **Song Seed Generator** — produces randomized songwriting prompts from four dimensions: Setting (location/time/direction), Song Type (Dylan-inspired taxonomy), Theme (Seligman & Peterson character strengths), and Color (120 Crayola crayon colors with swatch). These can be rolled individually or combined via "Generate Song Seed."
...
- `js/app.js` — all application logic in one file:
  - **Top**: data arrays (songTypes, themes, crayonColors, usStates, usCities, worldCities, days, directions)
  - **Middle**: generator functions (`generateSetting`, `generateType`, `generateTheme`, `generateColor`, `generateAll`)
