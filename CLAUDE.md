...
1. **Song Seed Generator** — produces randomized songwriting prompts from five dimensions: Setting (location/time/direction), Song Type (Dylan-inspired taxonomy), Theme (Seligman & Peterson character strengths), Color (120 Crayola crayon colors with swatch), and Dylan Words (7 random words from a curated Dylan vocabulary). These can be rolled individually or combined via "Generate Song Seed."
...
- `js/app.js` — all application logic in one file:
  - **Top**: data arrays (songTypes, themes, crayonColors, dylanWords, usStates, usCities, worldCities, days, directions)
  - **Middle**: generator functions (`generateSetting`, `generateType`, `generateTheme`, `generateColor`, `generateDylanWords`, `generateAll`)
